...
class LineEditSearch(QtWidgets.QLineEdit):
  ...
  def __init__(self, parent):
    ...
    """)
    self.model: Union["project_model.StorageSpaceModel", None] = None
    self.textChanged.connect(self.on_text_changed)
    self.search_dialog = _SearchDialog(parent)
    self.setFixedSize(275, 30)

  def add_model_to_search(self, a_model: Union["project_model.StorageSpaceModel"]):
    self.model: Union["project_model.StorageSpaceModel"] = a_model
...
class _SearchDialog(QtWidgets.QDialog):
  ...
  def update_search_results(self, results: list[QtGui.QStandardItem]):
    ...
    for tmp_result in results:
      tmp_workspace_project: "workspace_project.WorkspaceStorageSpace" = tmp_result.data(
  ...
  def emit_item_clicked(self) -> None:
    tmp_workspace_project: "workspace_project.WorkspaceStorageSpace" = self.list_widget.currentItem().data(model_definitions.RolesEnum.OBJECT_ROLE)
