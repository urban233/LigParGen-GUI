...
begin
  try
    ...
    if not RunPowerShellCommand('wsl --import almaLigParGen9 "C:\ProgramData\IBCI\wsl2\LigParGenGUI" "C:\ProgramData\IBCI\LigParGenGUI\temp\alma9-ligpargen-rootfs.tar"') then
    begin
      Result := False;
...
begin
  try
    ...
    if not RunPowerShellCommand('Remove-Item -Path "C:\ProgramData\IBCI\wsl2\LigParGenGUI" -Recurse -Force') then
    begin
      Result := False;
      Exit;  // Exit early if this command fails
    end;
